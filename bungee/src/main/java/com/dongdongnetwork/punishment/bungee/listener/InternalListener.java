...
public class InternalListener implements Listener {
    ...
    public void onPluginMessageEvent(PluginMessageEvent e) {
        if (!e.getTag().equals("dongdongpunishment:main")) {
            return;
        }
        ...
        switch (channel) {
            case "Punish":
                ...
                break;
            default:
                universal.debug("Unknown channel for tag \"DongDongPunishment\"");
                break;
        }
    }
    public void sendToBukkit(String channel, List<String> messages) {
        ...
        ProxyServer.getInstance().getServers().keySet().forEach(server -> ProxyServer.getInstance().getServerInfo(server).sendData("dongdongpunishment:main", out.toByteArray(), true));
    }
}